...
```{r}
voters_by_county <- read_csv("tabula-Eligible Active Voters by County - GP22 (1).csv")
...
**Answer** The precinct column is titled "x1" because of clean_names. There are two rows that show only NA values. There is a row, Bend, that was created because the name of the precinct "Sunrise Beach 2 & 3 and Wilson Bend" spilled over onto another row. Also, R is interpreting the "voters" and "turnout" columns as characters (<chr>) instead of numbers (<dbl>). In order to change the "turnout" column to numbers, we need to remove the percent signs.
...
**Answer** There is still a row titled "Jurisdiction Wide" where there is only NA, and there is still a row named "Bend" with only NA.
...
**Answer** Now the dataframe has 21 rows instead of 23 rows.

Also, we didn't get ride of rows where "voters" is NA because that would have gotten rid of the row for absentee ballots.
