...
Let's suppose we can't find a table like that for download, but we do see a version on the SBOE website at this URL: [https://elections.maryland.gov/elections/2022/general_results/gen_detail_results_2022_3_1.html](https://elections.maryland.gov/elections/2022/general_results/gen_detail_results_2022_3_1.html).
...
**Answer** 
The column names are kind of goofy, with spaces in the names of candidates and more spaces in between the names and the candidate's party affiliation in each column. The columns are also the wrong datatype - <chr> when they clearly contain numbers, even though those numbers have commas, which is a problem. And the last "Totals" row has percentages as well as totals.
```{r}
...
# Just keep the first dataframe in our list   <- But there's actually only one, right? Are we doing this just because if there were more than one dataframe in our list from the site, we would need to specify which one to use?
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2, 3 and 4 are referring to?
**Answer** 
They are referring to the numbers of the columns we are renaming, which is much easier to type than the whole column name.
```{r}
# Read in all html from table, get the HTML table. <- Why do we have to read in all the html from the table each time, when we've already saved it as a variable?
results <- ag_url %>%
  read_html() %>%
  html_table()
...
**Answer** While there are only 20 <li> tags displayed, the {xml_nodeset} part at the top suggests that there are actually 57 tags in the html source.

```{r}
cmte_results %>% html_elements('li')
```

Good news and bad news here: we have all of the <li> tags, but we have lots of them, and it's hard to see any of the committees. Let's see what we're dealing with and just look at the text *inside* the <li> tags. To do that, we add `html_text()` to the end of the last code block:

### Task 10b: Isolate the text of `li` tags inside the HTML using `html_text`
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output:
**Answer** 
The content in tags 1-20 and 50-57 - the ones we don't want - all begin with newline characters. Tags 21-49 contain the committee names that we are looking for.
```{r}
cmte_results %>% html_elements('li') %>% html_text()
```

Ok, so now we can see the committee names. But they are surrounded by a bunch of things we don't want, and this *still* isn't a dataframe. Let's solve the second problem first with the `as_tibble()` function:
...
**Answer** 
I see a dataframe showing all 57 rows with a single column. The ones that we don't want - rows 1-20 and 50-57 - begin with indentations. The committee names are in rows 21-49.
```{r}
committees <- cmte_results %>% html_elements('li') %>% html_text() %>% as_tibble()
```

Now we're getting closer. The rows in this dataframe we don't want begin with a newline character `\n`. Let's get rid of those using `filter()` and rename our single column:
...
committees <- committees %>% 
  # filter out rows that begin with a newline character
