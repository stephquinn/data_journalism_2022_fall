...
**Answer** It has 25 rows and 20 columns.
...
**Answer**
After telling R that we are using the md_voters dataframe, arrange(desc(TOTAL_DIFF)) arranges the rows (which are counties) in order of largest difference in total registered voters to smallest. select(County, TOTAL_DIFF) means that we only want to see the columns showing the Maryland counties and difference in total registered voters between 2020 and 2022. head(10) shows only the top 10 results, and maryland_top_counties displays the new dataframe.
```{r}
...
**Answer** I see the blank canvas, a plain gray rectangle full of possibility.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** I see a bar chart showing the comparative differences (increases) in the number of registered voters in the ten counties where those increases were largest, in no particular order. The x axis shows the counties, while the y axis shows the increases.
...
**Answer** Now the counties are arranged from the smallest increase in registered voters to the largest, showing that Frederick County had the largest increase in registered voters between 2020 and 2022 by a significant margin.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The columns represented on the x and y axes have switched, so that counties are on the y axis and increases are on the x axis. This has had the positive side effect of making the county names horizontal and spacing them out so they don't overlap.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
Intead of a light gray background with a white grid, there is a white background with a light gray grid.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Answer** There's a light blue background with white lines instead of a grid.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
There's a title: Maryland Counties with Biggest Increases in Voters. Now the x axis is labeled "net change," which is more descriptive, and the y axis is labeled "county." There's also a caption saying that the data came from the Maryland State Board of Elections.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see a tibble showing the number of contributions made on dates between April and June 2022.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The line chart shows me change over time, and now all the data is visualized for me at once. It's a very jagged line, but there's a smaller peak in the first half of May and a huge increase in the second half of June, just before the reporting deadline.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see the same line, but with more specific data labels for every week and corresponding grid lines to make it easier to determine when increases and decreases in numbers of donations occurred.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now the date labels are angled at 45 degrees, which spaces them out a bit and makes them a little easier to read.
```{r}
md_winred_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
