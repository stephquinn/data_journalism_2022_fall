...
**Answer** Warning: One or more parsing issues, see 'problems()' for detailsRows: 476915 Columns: 13
...
**Answer** This tibble is showing 1,640 problems. According to this result, R expected each of the row-column combinations listed to show a true/false, either/or outcome, but instead it found data points like "20," "21" and "15." R is not identifying these as numbers and has labeled them as characters (<chr>).
...
**Answer** I don't see any errors, so I guess parsing was successful.
...
**Answer** District is numbers. Precinct is characters. Mail is numbers.
```{r}
...
**Task** Run the following codeblock to change the data type of the "election_day" field from a character (chr) to a number, and then glimpse the data, to see the change. Add a description of what the mutate code does to your reference notebook.     
...
**Answer**: There are 402,345 results where mail is NA. I think this suggests that there were 402,345 times when there was no data entered in the field that would have shown the number of mail-in ballots cast for a candidate in a particular precinct. It's unclear what this actually means. Maybe these precincts didn't report mail-in ballots to whoever created this dataset. Or maybe they didn't have any mail-in ballots but they didn't report anything instead of reporting zero.
...
**Answer** There are 12 registrations for January 1900 and one for May 1900. It's unlikely that anyone in North Carolina is that old, so I would discard those results. From this tibble, I would judge that the voter who has been registered the longest in Yadkin County registered to vote in February 1933. That person must have been at least 18 then, so they would have to be at least 108. (Is that plausible?) After that, the results quickly go into the 1950s and 60s. There are hundreds of people who have been registered in Yadkin County since 1966, according to this data. If they were 18 at the time of registration, they would only be in their 70s. I would have expected more registrations that old. Either the elderly of Yadkin County are not very avid voters, or - and I think this is more likely - they don't show up because they've had to switch their registrations when they moved.
```{r}
yadkin_voters %>% 
...
**Answer** I'm curious what "request from voter" means. Does this just mean they decided they weren't going to vote anymore and asked to be removed from the rolls? Or did those people notify the county that they were moving so they could be taken off the rolls? 

Also, what does "temporary registrant" mean?

I think it would be interesting to analyze the "felony conviction" and "felony sentence completed" data. Does the county automatically get notified when people are convicted of felonies or finish their sentences? How likely are people who justed finished felony sentences to vote?

I'm also wondering what the difference between "confirmation" and "verification" is. How often does the county verify its registered voters?
...
**Answer** There are 14 rows in this data set.
...
**Answer** R is interpreting the contents of the "amount" field as characters, even though they seem to be numbers.
```{r}
glimpse(conowingo)
```
...
**Answer**1. The column headers are inconsistently styled. The "LAST_NAME" column header is in all caps, while the others are lowercase. The first column begins with a number. "address one" has a space instead of an underscore.
2. The address "118 Merry Knoll Lane/Ln" appears two ways. Sometimes Conowingo is all in caps; sometimes it's just conowing.
3. The zip field has 5-digit and 9-digit zip codes.
...
**Answer** clean_names put an "x" in front of the column header 1_linenumber so it no longer begins with a number. It also fixed the issues with the other headers (last_name is lowercase; address_one now has an underscore).
...
**Answer** The error says "Problem while computing 'total_amount = sum(amount)'" and "invalid 'type' (character) of argument." I think this is because R is interpreting the values in the amount column as characters rather than numbers. You can't add up characters, but you can find the sum of a group of numbers. 
...
**Answer** There are two identical rows. The name of the donor is Derrick Hamilton.
...
**Answer** Because one of the two identical Derrick Hamilton rows has been omitted, there are now 13 instead of 14 rows.
...
**Answer** Now the zip codes are all 21918.
...
I'm guessing it's supposed to be "Conowingo" instead of Arnold. Now all the Conowingos begin with a capital C with all the other letters lowercase. But str_tot_title() did not fix "Conowing" or "Conowingoo."
...
**Answer** Now there's no more "Conowing," but there's one more misspelling: "Conowingoo."
