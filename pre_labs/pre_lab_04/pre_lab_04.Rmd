...
**Answer** Yes - the column names are the same, but the columns are in a different order in the 2020 data. The 2020 data has GRN before LIB rather than the other way around, as well as OTH before UNA rather than the other way around.
...
**Answer** There are now 24 rows in the joined data, while county_voters_2020 had 25 rows. The 25th row in the 2020 voter registration data showed total numbers of Democrats, Republicans, etc. There is no corresponding total in the population data - just the list of 24 counties. join() automatically omits any rows that aren't in both datasets being joined.
...
**Answer** I don't seem to get an error code, but I did notice that there is no "NAME" column in the joined table. I think this must be because of the by=c("COUNTY"="NAME") part of the code. Does that tell R to match the "NAME" column from maryland_demographics to the "COUNTY" column from maryland_population_with_voters? What if the rows were in a different order in one of the datasets?
```{r}
...
**Answer** Prince George's County has the highest 18+ Black population as a percentage of the entire population over 18, at 60.9%. Prince George's County also has the highest percentage of Democrats - 78.3% - of any county.
...
**Answer** Howard County has the highest 18+ Asian population as a percentage of the whole 18+ population, at 19.4%. Howard County has 52.4% Democrats. The percentage of the voting-age population that is Asian seems to be a less reliable indicator of how Democratic a county leans compared to the same number for counties' Black populations. Only seven counties have more than 4% Asians in their voting-age populations, and they vary from 38.9% to 78.3% Democrats.
...
**Answer** Now it's characters (<chr>)!
