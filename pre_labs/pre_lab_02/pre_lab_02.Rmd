...
**Answer** The code is calculating the percentage election day votes each candidate got in specific counties and placing those results in a new column, percent_election_day. It's calculating these percentages by dividing the number of election day votes by total votes for each candidate in a given county. Because the code is selecting "office, district, name_raw, party, jurisdiction, election_day, votes," the tibble shows only these columns and the new column, percent_election_day..
...
**Answer** While the tibble from the previous task has not arranged the rows in any particular order, the more recent tibble shows candidates' performance in particular counties in order of highest percentage election day votes to lowest percentage election day votes.  It changed in this way because after we calculated percent_election_day using mutate, we arranged the result in order of descending percentage election day votes with arrange(desc(percent_election_day)).
...
**Answer** 
Yes, I think this tibble is more useful. The first tibble spotlights candidates who got very few votes in counties with relatively small populations. In the second tibble, the numbers of votes are larger and thus show the behavior of more people. Also, the percentages on the first page of the second tibble are strikingly low. If 50% or 60% of people were voting mail-in even before the pandemic, it's probably worth asking why. It's also striking that the candidates who got the highest percentages of mail-in votes in these counties were almost all Democrats.

I also thought it was interesting that rural counties dominated the tibbles showing both candidates who got the most election day votes and those who got the least election day votes.
...
**Answer** It looks like there are various string functions. 
str_to_lower() converts the string to lowercase.
str_length() figures out the length of a string.
...
**Answer** As I understand it, the code used case_when to create a new column, amount_category, to label contributions by the size of the contribution amount. case_when sets the criteria for labeling individual records in particular ways. If the contribution was less than $100, "Less than $100" will appear in the amount_category column for that contribution. If the contribution was between $100 and $500 (including $500), "Between $100 and $500" will appear in the amount_category column. And so on. Then the code used the head function to show the first six rows of the tibble.
...
**Answer** The largest category in terms of total dollar amount is "Less than $100," with $868,921.80.
...
**Answer** Making a new variable, selected_prince_georges, is important because it saves the work that the "select" function so that it appears in the environment and we can ask more questions of the tibble we just made using additional R verbs.
...
Ah, ok. So is "nrow" showing us how many rows the tibble would have? Why didn't this codeblock give us a tibble?
...
**Answer:** The expenditure_date column now shows that the information in the column are dates. The datatype displayed beneath "expenditure_date" is <date> for date, instead of <chr> for character. The code has also formatted the dates into y-m-d order.
...
**Answer** My guess is that floor_date might have rounded each piece of data down to the nearest month (which is why the "month" column shows the first of each month) so that we could then use group_by and summarize to show the dollar amounts contributed in each month.
