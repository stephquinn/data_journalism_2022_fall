...
```
```{r}
results_2018 <- read_xlsx("~/Documents/GitHub/data_journalism_2022_fall/labs/2018_results.xlsx", sheet = 2)
...
```{r}
combined_results <- inner_join(results_2018, results_2022, by = "county")
```
...
combined_results <- combined_results %>% 
  mutate(mcmullin_wilson_difference = mcmullin - wilson)
```
...
```{r}
combined_results <- combined_results %>% 
  mutate(lee_romney_difference = lee - romney)
```
...
**A3** 
On what day did Cardin issue the most press releases? What month had the most press releases? 
What are the longest and shortest press release titles?
Are there certain parts of Maryland that were mentioned much more often than others in the press release titles? 
What are the most common words and phrases in the titles (other than a, and, the, etc.)?
How often did press release titles refer to Virginia versus Maryland's other neighbors?
Which prominent politicians from Maryland or nearby states/jurisdictions were mentioned most often in press release titles?
Do the press release titles suggest Cardin has any favorite international causes?
```{r}
...
dates <- results %>% html_elements('h5') %>% html_text %>% as_tibble()
```
```{r}

titles <- results %>% html_elements('h3 a') %>% html_text() %>% as_tibble()

```
```{r}
urls <- results %>% html_nodes('h3 a') %>% html_attr('href') %>% as_tibble()
```

```{r}
cardin_prs <- bind_cols(dates, titles, urls)
```
```{r}
cardin_prs <- cardin_prs %>% 
  rename(date = "value...1",
         title = "value...2",
         url = "value...3")
```
